...
async function decodeEvent() {
    ...
    if (!found) {
        eventResult.value =
            "No matching event found in ABI for the given event data.";
    }
}
...
async function decodeError() {
    const selector = error.value.slice(0, 10); // '0x' + 4 bytes(8 hex chars)
    ...
    for (const item of items) {
        ...
        if (!found) {
            errorResult.value =
                "No matching error found in ABI for the given error data.";
        }
    }
}
...
async function decodeFunctionData() {
    ...
    if (!found) {
        functionDataResult.value =
            "No matching function found in ABI for the given function data.";
    }
}
    ...
    <div class="app-container">
        <header class="app-header">
            ...
            <p class="app-subtitle">
                Easily decode errors, events, and function data from
            ...
            <section class="card abi-section">
                ...
                <div class="card-body">
                    <textarea
                        ...
                        @blur="parseABI"
                    />
                    <div v-if="abiError != ''" class="error-message">
                        {{ abiError }}
                    </div>
                </div>
            </section>
